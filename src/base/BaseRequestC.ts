...
import HttpAxios from "@/extend/axios"
import {
    axiosReqConfig,
    chainConfig,
    callBlackConfig,
    ...
    protected success(result: Result) {
        if (result.result) {
            ...
            if (result.code != 0) {
                ...
                if (ErrResult.needRouter == true) {
                    if (ErrResult.needMsg == true) {
                        ElMessage.error(ErrResult.msg)
                    }
                    if (ErrResult.router != undefined) {
                        router.push(ErrResult.router)
                    }
                } else {
                    return ErrResult
                }
